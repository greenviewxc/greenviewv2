    ...
    if (group == 1) {
      setObjects((prevObjects) => [
        ...prevObjects,
        {
          ...
          y: -window.innerHeight / 2, // start at the top
          path: source[group - 1], // random image
        },
      ]);
    } else {
      setObjects((prevObjects) => [
        ...
        ...Array.from({ length: count }, (_, index) => ({
          id: prevObjects.length + index + 1, // Unique ID for each new object
          group: group,
          x: Math.floor(Math.random() * -500) - 200, // Random x position
          y: Math.random() * (max - min) + min, // Start at the top
          path: source[group - 1], // Random image
        })),
      ]);
    }
